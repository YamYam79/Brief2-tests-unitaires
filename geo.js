...
// Une fonction qui permet de réaliser tous lests dans Geo.js
function calculateDistance(start, end) {
//   Vérifier si les deux entrées sont des tableaux de longueur 2(latitude and longitude)
  if (
    !Array.isArray(start) ||
    !Array.isArray(end) ||
    start.length !== 2 ||
    ...
    throw new Error(
      "L'entrée doit être constituée de deux tableaux, contenant chacun exactement deux nombres (latitude and longitude)."
    );
  }
  ...
  if (
    ...
    throw new Error("L'entrée doit être constituée de deux nombres");
  }

  // Convert degrees to radians
  const toRadians = (degrees) => (degrees * Math.PI) / 180;

  // Earth's radius in meters = rayon de la terre en 
  const R = 6371000;
